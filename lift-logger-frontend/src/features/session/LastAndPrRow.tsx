...
/**
 * One-shot form of the LAST + PR lookup. Exported for callers that seed
 * form state at a discrete moment (AddBlockOverlay prefills targets when an
 * exercise is picked) rather than reactively — a live query would stomp the
 * user's manual edits on every re-emit.
 */
export async function queryLastAndPr(
  exerciseId: string,
  currentSessionId: string | null,
): Promise<{ last: LastSet | null; pr: Pr | null }> {
...
export function useLastAndPr(
  ...
  const data = useLiveQuery(
    () => queryLastAndPr(exerciseId, currentSessionId),
